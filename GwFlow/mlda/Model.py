...
def project_eigenpairs(model_fine, model_coarse):
    ...
    """
    model_coarse.random_process.eigenvalues = np.zeros(model_fine.mkl)
    model_coarse.random_process.eigenvectors = np.zeros((model_coarse.random_process.eigenvectors.shape[0], model_fine.mkl))
    
    model_coarse.random_process.eigenvalues[:] = \
        model_fine.random_process.eigenvalues
    for i in range(model_fine.mkl):
